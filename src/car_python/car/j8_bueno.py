...
class TerrainPPOTrainer(Node):
    ...
    def build_meta_policy(self):
        ...
        G = tf.keras.Input(shape=(BIG_PATCH, BIG_PATCH, 5), name="big_patch")
    ...
    def build_meta_value(self):
        ...
        """
        G = tf.keras.Input(shape=(BIG_PATCH, BIG_PATCH, 5), name="big_patch_val")
    ...
    def extract_big_patch(self):
        ...
        # frontier entropy layer
        entropy_layer = np.zeros((BIG_PATCH, BIG_PATCH), np.float32)
        for f, ent in self.frontier_entropy.items():
            ...
            if 0<=gi<BIG_PATCH and 0<=gj<BIG_PATCH:
                entropy_layer[gj,gi] = min(1.0, ent)
        big_patch = np.dstack([patch_down, goal_layer, entropy_layer]) #(64.64),4,
...
#### cambiado para usar las entropia y ayudarse la eleccion de las frontereas 
    ...
    def select_frontier(self, cp, grid, info):
        ...
        return best, "FRONTIER" if best is not None else (None, "NONE")
...
### old no tiene en cuenta la nueva capa de entropia
