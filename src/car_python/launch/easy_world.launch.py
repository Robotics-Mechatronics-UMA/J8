...
def generate_launch_description():
    ...
    goal_yaml = PathJoinSubstitution([
        ...
        'config',                       # subcarpeta
        'world1.yaml'                     # nombre del archivo
    ])
    ...
    urdf_spawn_node = Node(
        ...
        arguments=[
            ...
            '-x', '-26.8',   # Initial X position
            '-y', '39.4',   # Initial Y position
            '-z', '3.35',  # Initial Z position (adjust if needed)
            '-Y', '-1.5' # Initial yaw (rotation around Z axis)
        ],
        output='screen'
    )
